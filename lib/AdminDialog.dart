                ...
                children: <Widget>[
                  ...
                  Row(
                    ...
                    children: <Widget>[
                      InkWell(
                        onTap: () async{
                          if(snapshot.data.data()['status']!=status[3]){
                            await ComplaintFiling().complaints.doc(widget._complaintID).update({'status': status[3]});
                            for(var doc in await users.get().then((value) => value.docs)){
                              if(doc.data()['bookmarked'].contains(widget._complaintID) || doc.data()['list of my filed Complaints'].contains(widget._complaintID)){
                                await users.doc(doc.id).update({'notification':FieldValue.arrayUnion([{'complaintID': widget._complaintID,'time': DateTime.now().toString()}])});
                              }
                            }
                          }
                        },
                      ...
                      InkWell(
                        onTap: () async{
                          if(snapshot.data.data()['status']!=status[2]){
                            await ComplaintFiling().complaints.doc(widget._complaintID).update({'status': status[2]});
                            for(var doc in await users.get().then((value) => value.docs)){
                              if(doc.data()['bookmarked'].contains(widget._complaintID) || doc.data()['list of my filed Complaints'].contains(widget._complaintID)){
                                await users.doc(doc.id).update({'notification':FieldValue.arrayUnion([{'complaintID': widget._complaintID,'time': DateTime.now().toString()}])});
                              }
                      ...
                      InkWell(
                        onTap: () async{
                          if(snapshot.data.data()['status']!=status[4]){
                            //await snapshot.data.data().update('status', (value) => status[4]);
                            await ComplaintFiling().complaints.doc(widget._complaintID).update({'status': status[4]});
                            for(var doc in await users.get().then((value) => value.docs)){
                              if(doc.data()['bookmarked'].contains(widget._complaintID) || doc.data()['list of my filed Complaints'].contains(widget._complaintID)){
                                await users.doc(doc.id).update({'notification':FieldValue.arrayUnion([{'complaintID': widget._complaintID,'time': DateTime.now().toString()}])});
                              }
