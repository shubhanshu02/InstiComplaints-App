...
class _ComplaintDialogState extends State<ComplaintDialog> {
  ...
  final CollectionReference _complaints = FirebaseFirestore.instance.collection('complaints');
  ...
  void initState() {
    super.initState();
    setState(() {
      _userDocument.get().then((value) async {bookmarked= await value.data()['bookmarked'].contains(widget._complaintID);});
      print(bookmarked);
    });
  }
                    ...
                    children: <Widget>[
                      Row(
                        children: <Widget>[
                          ...
                          IconButton(
                            onPressed: ()async{
                              if(await _userDocument.get().then((value) => value.data()['bookmarked'].contains(widget._complaintID))){
                                await _userDocument.update({'bookmarked':FieldValue.arrayRemove([widget._complaintID])});
                                setState(() {
                                  bookmarked =false;
                                });
                              }
                              else{
                                await _userDocument.update({'bookmarked':FieldValue.arrayUnion([widget._complaintID])});
                                setState(() {
                                  bookmarked=true;
                                });
                              }
                      ...
                      Container(
                        height: snapshot.data.data()['list of Images'].length!=0?(4.2 * MediaQuery.of(context).size.height) / 10 : (2 * MediaQuery.of(context).size.height) / 10,
                        child: ListView(
                              ...
                              child: Column(
                                ...
                                children: <Widget>[
                                  IconButton(
                                    ...
                                    onPressed: (){
                                      if(snapshot.data.data()['upvotes'].contains(FirebaseAuth.instance.currentUser.uid)){
                                        _complaints.doc(widget._complaintID).update({'upvotes':FieldValue.arrayRemove([FirebaseAuth.instance.currentUser.uid])});
                                      }
                                      else{
                                        _complaints.doc(widget._complaintID).update({'upvotes':FieldValue.arrayUnion([FirebaseAuth.instance.currentUser.uid])});
                                      }
                                    },
                                    color: snapshot.data.data()['upvotes'].contains(FirebaseAuth.instance.currentUser.uid) ? Colors.blue[400] : Colors.grey,
                                    iconSize:
                                        (0.35 * MediaQuery.of(context).size.height) /10,),
                                  Text(
